'''
@author Alessia Bussard
@description Server-side EmotiBit heart rate processor

To use: current_heart_rate = processor.get_current_heart_rate()
'''

import threading
import time
import numpy as np
from scipy import signal
from pythonosc import dispatcher
from pythonosc import osc_server
from model.PlayerPool import PlayerPool
from model.GameHandler import GameHandler


class EmotiBitProcessor:
    """
    Server-side processor for EmotiBit data.
    Calculates heart rate and updates player objects.
    """

    # Singleton instance
    _instance = None

    @staticmethod
    def get_instance():
        """Get or create the singleton instance."""
        if EmotiBitProcessor._instance is None:
            EmotiBitProcessor._instance = EmotiBitProcessor()
        return EmotiBitProcessor._instance

    def __init__(self):
        """Initialize the EmotiBit processor."""
        # Ensure this is only initialized once
        if EmotiBitProcessor._instance is not None:
            raise Exception("This class is a singleton. Use get_instance() instead.")

        # Data processing variables
        self.ppg_green_buffer = []
        self.ppg_timestamps = []
        self.latest_hr = 0
        self.sampling_rate = 25  # From EmotiBit config
        self.buffer_duration = 5  # Seconds of data to keep
        self.buffer_size = int(self.sampling_rate * self.buffer_duration)

        # Thread control
        self.running = False
        self.osc_server = None
        self.hr_calc_thread = None
        self.server_thread = None

        # OSC server settings
        self.ip = "0.0.0.0"  # Listen on all interfaces
        self.port = 12345  # EmotiBit OSC port

        # Player mapping (device_id -> player_name)
        # This allows associating EmotiBit devices with players
        self.player_mapping = {}

        # Initialize the dispatcher
        self.disp = dispatcher.Dispatcher()
        self.disp.map("/EmotiBit/0/PPG:GRN", self.ppg_green_handler)
        self.disp.set_default_handler(self.default_handler)

        print("EmotiBit processor initialized")

    def start(self):
        """Start the EmotiBit processor."""
        if self.running:
            return

        self.running = True

        # Start the heart rate calculation thread
        self.hr_calc_thread = threading.Thread(target=self.calculate_heart_rate)
        self.hr_calc_thread.daemon = True
        self.hr_calc_thread.start()

        # Start the OSC server in a separate thread
        self.server_thread = threading.Thread(target=self.run_osc_server)
        self.server_thread.daemon = True
        self.server_thread.start()

        print(f"EmotiBit processor started on {self.ip}:{self.port}")

    def stop(self):
        """Stop the EmotiBit processor."""
        if not self.running:
            return

        self.running = False

        if self.osc_server:
            self.osc_server.shutdown()

        print("EmotiBit processor stopped")

    def run_osc_server(self):
        """Run the OSC server to receive EmotiBit data."""
        try:
            self.osc_server = osc_server.ThreadingOSCUDPServer((self.ip, self.port), self.disp)
            print(f"Listening for EmotiBit OSC data on {self.ip}:{self.port}")
            self.osc_server.serve_forever()
        except Exception as e:
            print(f"Error in OSC server: {e}")
            if self.running:
                # Try to restart the server after a delay
                time.sleep(5)
                if self.running:
                    self.run_osc_server()

    def ppg_green_handler(self, address, *args):
        """Handle incoming green PPG data."""
        if args and len(args) > 0:
            self.ppg_green_buffer.append(args[0])
            self.ppg_timestamps.append(time.time())

            # Keep buffer at defined size
            if len(self.ppg_green_buffer) > self.buffer_size:
                self.ppg_green_buffer = self.ppg_green_buffer[-self.buffer_size:]
                self.ppg_timestamps = self.ppg_timestamps[-self.buffer_size:]

    def default_handler(self, address, *args):
        """Default handler for other OSC messages."""
        # Extract device ID from address if needed
        # Example: /EmotiBit/DEVICE_ID/PPG:GRN
        parts = address.split('/')
        if len(parts) >= 3:
            device_id = parts[2]
            # Could be used to map to specific players if multiple EmotiBits

    def calculate_heart_rate(self):
        """Calculate heart rate from PPG data."""
        min_data_points = self.sampling_rate * 2  # At least 2 seconds of data

        while self.running:
            # Check if we have enough data
            if len(self.ppg_green_buffer) >= min_data_points:
                try:
                    # Get the signal as numpy array
                    green_signal = np.array(self.ppg_green_buffer[-min_data_points:])

                    # Normalize the signal
                    green_signal = (green_signal - np.mean(green_signal)) / np.std(green_signal)

                    # Apply bandpass filter (0.5-3.5 Hz = 30-210 BPM)
                    b, a = signal.butter(3, [0.5 / (self.sampling_rate / 2), 3.5 / (self.sampling_rate / 2)],
                                         btype='band')
                    filtered_green = signal.filtfilt(b, a, green_signal)

                    # Find peaks in the filtered signal
                    peaks, _ = signal.find_peaks(filtered_green, distance=int(self.sampling_rate * 0.5))

                    if len(peaks) >= 2:
                        # Calculate heart rate from peak intervals
                        peak_intervals = np.diff(peaks)
                        mean_interval = np.mean(peak_intervals)

                        # Convert to beats per minute
                        hr_bpm = 60 * self.sampling_rate / mean_interval

                        # Sanity check - HR should be between 40-180 BPM for most adults
                        if 40 <= hr_bpm <= 180:
                            self.latest_hr = hr_bpm
                            print(f"Current heart rate: {hr_bpm:.1f} BPM")

                            # Update heart rate for all active players
                            self.update_players_heart_rate(hr_bpm)
                except Exception as e:
                    print(f"Error calculating heart rate: {e}")

            # Sleep to avoid using too much CPU
            time.sleep(0.2)

    def update_players_heart_rate(self, heart_rate):
        """Update heart rate for all active players."""
        try:
            # Get the game handler
            game_handler = GameHandler()

            # If game is active, update player heart rates
            if game_handler and game_handler.get_game():
                game = game_handler.get_game()

                # Update player1 heart rate
                if game.get_player1():
                    game.get_player1().set_heart_rate(heart_rate)

                # Update player2 heart rate
                if game.get_player2():
                    game.get_player2().set_heart_rate(heart_rate)

                print(f"Updated heart rate for all players: {heart_rate:.1f} BPM")
        except Exception as e:
            print(f"Error updating player heart rates: {e}")

    def register_device(self, device_id, player_name):
        """Register an EmotiBit device with a player name."""
        self.player_mapping[device_id] = player_name
        print(f"Registered EmotiBit device {device_id} for player {player_name}")

    def get_current_heart_rate(self):
        """Get the most recently calculated heart rate."""
        return self.latest_hr